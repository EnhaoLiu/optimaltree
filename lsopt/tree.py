...
from ._base import solve_oct_MILP, solve_oct_MILP_OLD, ModelTree, get_solution_from_CART
...
class OptimalTreeClassifier:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # Get warm start solutions
        if self.warm_start:
            a, b, d, z, l, c, Loss, NN, N = get_solution_from_CART(X=X_transformed,
                                                                   y=y,
                                                                   scaler_X=None,
                                                                   max_depth=self.max_depth,
                                                                   min_samples_leaf=self.min_samples_leaf
                                                                   )
            self.solver_options['warm_start'] = self.warm_start
            self.solver_options['a'] = a
            self.solver_options['b'] = b
            self.solver_options['d'] = d
            self.solver_options['z'] = z
            self.solver_options['l'] = l
            self.solver_options['c'] = c
            self.solver_options['Loss'] = Loss
            self.solver_options['NN'] = NN
            self.solver_options['N'] = N

        # Solve Optimal Classification Tree
        model, run_time, solution_condition = solve_oct_MILP(X_transformed=X_transformed,
        ...
        self.solution_condition = solution_condition
    ...
    def _validate_X_predict(self, X, check_input):
        ...
        if self.tree_.feature_removed_idx.size > 0:
            X = np.delete(arr=X, obj=self.tree_.feature_removed_idx, axis=1)
...
# %%
class BinNodePenaltyOptimalTreeClassifier:
    """An Optimal Tree Classifier.
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # Solve Optimal Classification Tree
        model, run_time, solution_condition = solve_oct_MILP_BIN(X_transformed=X_transformed,
                                                                 y_transformed=y_transformed,
        ...
        # Construct ModelTree object
        tree_ = ModelBinTree(model=model,
                             scaler_X=scaler_X,
                             feature_removed_idx=feature_removed_idx,
                             classes=self.classes_,
                             criterion=self.criterion,
...
# %%
class OldOptimalTreeClassifier:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        # Solve Optimal Classification Tree
        model, run_time, solution_condition = solve_oct_MILP_OLD(X_transformed=X_transformed,
                                                                 y_transformed=y_transformed,
                                                                 L_hat=L_hat,
                                                                 epsilons=epsilons,
                                                                 alpha=self.alpha,
